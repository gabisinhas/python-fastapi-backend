"""Configuration for Pydantic database."""
...
class ConfigDict(TypedDict, total=False):
    ...
    frozen: bool
    """
    Whether database are faux-immutable, i.e. whether `__setattr__` is allowed, and also generates
    ...
    """
    Whether to populate database with the `value` property of enums, rather than the raw enum.
    ...
    """
    Whether to build database and look up discriminators of tagged unions using python object attributes.
    """
    ...
    ///
    """
    # whether instances of database and dataclasses (including subclass instances) should re-validate, default 'never'
    revalidate_instances: Literal['always', 'never', 'subclass-instances']
    """
    When and how to revalidate database and dataclasses during validation. Can be one of:

    - `'never'`: will *not* revalidate database and dataclasses during validation
    - `'always'`: will revalidate database and dataclasses during validation
    - `'subclass-instances'`: will revalidate database and dataclasses during validation if the instance is a
        subclass of the model or dataclass

    The default is `'never'` (no revalidation).

    This configuration only affects *the current model* it is applied on, and does *not* populate to the database
    ...
    """
    A tuple of strings and/or regex patterns that prevent database from having fields with names that conflict with its existing members/methods.
    ...
    [validation](../concepts/database.md#validating-data) and [serialization](../concepts/serialization.md#serializing-data) methods.
    ...
    This can be useful to avoid the overhead of building database which are only
    used nested within other database, or when you want to manually define type namespace via
